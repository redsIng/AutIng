...
set(st,'String',getString(message('Simulink:dee:StatusWorking')));   % set status string to working...
drawnow;
...
if BadHandle, % check for missing block and/or system.
  ...
  if SystemGone,  % give up for now (a destroy callback for blocks will fix all this).
    set(st,'String',getString(message('Simulink:dee:StatusParentSystemIsMissing',the_system_name)));
  ...
  if BlockMissing, % give up for now (a destroy callback for blocks will fix all this).
    set(st,'String',getString(message('Simulink:dee:StatusBlockIsMissing',figtitle)));
    set(fig,'Pointer','Arrow');
    drawnow;
    return
  end
...
theblockname = get_param(sysname,'MaskDisplay');
if isempty(theblockname),
  errstr = sprintf(getString(message('Simulink:dee:DEEBlockRenamedOrClosed',sysname)));
  errstr = str2mat(errstr,'     ',...
      getString(message('Simulink:dee:errorYouCan')),...
      ['  ',getString(message('Simulink:dee:ChangeBlockNameBack'))],...
      ['  ',getString(message('Simulink:dee:ReopenSystemifClosed'))],...
      ['  ',getString(message('Simulink:dee:RebuildBlockFromScratch'))]);

  set(st,'String',getString(message('Simulink:dee:StatusNOTFOUND',sysname)));
  errordlg(errstr,getString(message('Simulink:dee:dlg_DEEError')));
else
...
errorstr1 = getString(message('Simulink:dee:StatusOfInitialConditionsOfStates'));
errorstr2 = getString(message('Simulink:dee:StatusOfInputsUndefined'));
errorstr3 = getString(message('Simulink:dee:SimulationRunningCannotAlterSystemStructure'));

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% update total readouts located at the bottom of the edit field
%  for the system equation and initial condition blocks
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
set(tseto,'String',getString(message('Simulink:dee:NumberOfStates',num2str(numse))));
set(ticto,'String',getString(message('Simulink:dee:StatusTotal',num2str(numic))));
% affect status field
if (numse < numic),
...
elseif (numse > numic),
  ...
  set(st,'String',getString(message('Simulink:dee:StatusREADY')));
elseif numse==numic,
  set(st,'String',getString(message('Simulink:dee:StatusREADY')));
end
...
switch get_param(bdroot(sysname),'SimulationStatus'),
  case { 'initializing','running','paused','terminating' },
    if ((oldnumse~=numse) | (oldnumso~=numso) | (oldnumni~=numni)),
      errordlg(errorstr3,getString(message('Simulink:dee:dlg_DEEError')),'on');
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% stuff the new strings into the UserData matrices for the edit fields
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
checkinput  = dee_find_system(sysname,'SearchDepth',1,'BlockType','Inport');
checkoutput = dee_find_system(sysname,'SearchDepth',1,'BlockType','Outport');
checkstate  = dee_find_system(sysname,'SearchDepth',1,'BlockType','Integrator');
