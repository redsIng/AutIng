...
% find all left and right parentheses and brackets
lp = find(estring=='(');
rp = find(estring==')');
lb = find(estring=='[');
rb = find(estring==']');
...
if (length(lp) ~= length(rp)),
   ...
   set(errhan,'String','Status:  parens don''t match')
   return
end
...
if ~isempty(lp) & ~isempty(rp),
   if lp(1) > rp(1),
      %  status  error
      set(errhan,'String','Status:  first paren is not a left paren')
      return
   end

   if lp(length(lp)) > rp(length(rp)),
      %  status  error
      set(errhan,'String','Status:  last paren is not a right paren')
      return
   end
end
...
%    x's and find the corresponding right parens and brackets
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
lpind = [(x1ind+1),(x2ind+2)];
lbind = [(x3ind+1),(x4ind+2)];

% force lpind and lbind to be row vectors - this is done for
%    dimension purposes when building the lpmat and lbmat
lpind = lpind(:)';
lbind = lbind(:)';
...
if ~isempty(lind) && ~isempty(rind) && any((rind - lind) == 1),
  % status error
