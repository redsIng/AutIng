...
set(st,'String',getString(message('Simulink:dee:StatusLinearizingModel')));
drawnow;
...
NumOfInputs = size(get_param(FooBlk,'InputPorts'),1);
NumOfOutputs = size(get_param(FooBlk,'OutputPorts'),1);

if (NumOfInputs ~= size(u0val(:),1)),
   errordlg(getString(message('Simulink:dee:dlgLengthOfUMustEqualNumSystemInputs')),getString(message('Simulink:dee:dlgTitleLinearization')));
   set(st,'String',prevstr);
   drawnow;
   close_system(FooSys,0);
   return;
...
CatchStr =  ['errordlg(''', getString(message('Simulink:dee:dlgDEESystemSyntaxError')),''',''',getString(message('Simulink:dee:dlgTitleLinearization')),''');'];
eval(CmdStr,CatchStr);

if ~exist('tt'),
  set(st,'String',getString(message('Simulink:dee:StatusSyntaxError')));
  drawnow;
  close_system(FooSys,0);
  return
end
...
set(linb,'String',getString(message('Simulink:dee:LabelRestore')));
set(linb,'UserData',0);
