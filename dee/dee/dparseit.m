...
% an1ind = alphanumeric index for first index vector
an1ind = x1ind - 1;
...
% an2ind = alphanumeric index for first index vector
an2ind = x2ind - 1;
...
% an3ind = alphanumeric index for first index vector
an3ind = x3ind - 1;
...
% filter out those indices with an alphanumeric char just before the x in
%    x4ind
% an4ind = alphanumeric index for first index vector
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
an4ind = x4ind - 1;
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%  check to see that there are no parenthesis / bracket problems
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

% find ALL left and right parenthesis and brackets
lp = find(estring=='(');
rp = find(estring==')');
lb = find(estring=='[');
rb = find(estring==']');
...
% check to make sure that the left and right parens and brackets match
if (length(lp) ~= length(rp)),
   % do that status  thing
   disp('error - parens don''t match') 
   return
end
...
% make sure that the first paren is a left paren and that the first bracket
%   is a left bracket and make sure that the last paren and last bracket are
%   right orientated

if ~isempty(lp) & ~isempty(rp),
   if lp(1) > rp(1),
      %  status  error
      disp('first paren is not a left paren')
      return
   end

   if lp(length(lp)) > rp(length(rp)),
      %  status  error
      disp('last paren is is not a right paren')
      return
   end
end
...
if ~isempty(lb) & ~isempty(rb),
   if lb(1) > rb(1),
      %  status  error
      disp('first bracket is not a left paren')
      return
   end

   if lb(length(lb)) > rb(length(rb)),
      %  status  error
      disp('last bracket is not a right paren')
      return
   end
end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%  build index vectors for left parens and brackets corresponding to valid 
%    u's and find the corresponding right parens and brackets
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% find corresponding right parens
if ~isempty(lpind) 
...
%***************************************************************************%
% find out if the values between parens/brackets are integers as            %
%     they have to be                                                       %
%***************************************************************************%
lind = [lpind,lbind];
rind = [rpind,rbind];
...
if ~isempty(indmat),
   ...
   % remove all indices in index vectors corresponding to input u's 
   xind(badmask) = [];
   indmat(:,(badmask)) = [];
   rind(badmask) = [];
   lind(badmask) = [];
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% build qinds to hold all the values between valid parens and check args
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
   % convert arg strings to valid u's to numbers, subtract offset n, and 
