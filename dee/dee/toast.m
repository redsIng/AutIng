function toast(varargin)
%TOAST Destroy the contents of a DEE system.
%   TOAST is used when the underlying system for the DEE control panel
%   has been corrupted.
...
assumed_block_name = get_param(figtitle,'MaskDisplay');
...
switch SimState,
  case { 'initializing','running','paused','terminating' },
      errordlg(getString(message('Simulink:dee:CannotRebuildDuringSimulation')),getString(message('Simulink:dee:dlg_DEEError')),'on');
  ...
  otherwise
    ...
    if isempty(assumed_block_name),
      set(st,'String',getString(message('Simulink:dee:StatusDEEBlockNotPresent')));
      drawnow;
    else
      ...
      %  emanate from the same block but one of them has no sink block.  By deleting
